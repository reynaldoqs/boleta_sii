        ...
        <v-row>
          <v-col cols="5">
            <template v-if="!isLoading">
              <v-switch
                v-model="formData.receptorActuaReteniendo"
                :label="formData.receptorActuaReteniendo?'El receptor de la boleta actuará reteniendo el porcentaje vigente de Pago Provisional Mensual':'El contribuyente emisor será quien se encargue de la retención del porcentaje vigente de Pago Provisional Mensual'"
                class="mx-4"
              ></v-switch>
              ...
              <v-text-field
                v-model="formData.domicilioReceptor"
              ...
              <v-row>
                <v-col cols="8">
                  <v-text-field
                    v-model="formData.prestaciones[0].prestacion"
                    label="Prestación 1"
                    required
                  ></v-text-field>
                </v-col>
...
export default {
  ...
  data: () => ({
    ...
    formData: {
      ...
      razonReceptor: "victor espinoza",
      domicilioReceptor: "calle 756t",
      region: null,
      comuna: null,
      prestaciones: [{ prestacion: null, valor: null }],
    },
    response: null,
  }),
  computed: {
  ...
  methods: {
    ...
    async generar() {
      try {
        this.response = null;
        this.isLoading = true;
        const { data } = await genBoleta(this.formData);
        console.log(data);
        this.response = data;
